<!--sessions required for admin with role1-->
<?php
if(isset($_SESSION['login_username'])&&isset($_SESSION['login_role'])==1){ //checks for the session and if the session doesn't exist, it returns to the landing page
     ...
     if ($count>=1){
         while ($row=mysqli_fetch_assoc($result)){ //can fetch the datas from the table
             ...
             $role=$row['role'];
             if ($role==1){ //creating sessions if the role is of admin, else redirect to homepage
                 $_SESSION['uid']=$uid;
